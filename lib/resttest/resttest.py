...
def request(uri, method="GET", headers={},body=None, 
            ...
            request_transformers=[], response_transformers=[],predicates=[], error="test failed"):
    c = Http()
    uri, method, headers, body, redirections, connection_type = \
            loop_f(request_transformers,uri, method, headers, body, redirections, connection_type)
    ...
    def _dispatch(self, *args, **kwargs):
        ...
        for topic in topics:
            ...
            for function in desc.get(topic,()):
                f,f_keys = function
                f_args = tuple([kwargs[a] for a in f_keys if kwargs.has_key(a)])
